...
def _engine_extra(req) -> dict:
    return {k: getattr(req, k, None) for k in _ENGINE_EXTRA_FIELDS}
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        log.exception("failed to load OmniVoice engine")
        raise

    app.state.engine = engine
    log.info(
        "engine ready: model=%s device=%s dtype=%s sample_rate=%d languages=%d",
        settings.omnivoice_model,
        engine.device,
        engine.dtype_str,
        engine.sample_rate,
        len(engine.list_languages()),
    )

    yield


app = FastAPI(title="omnivoice-open-tts", version="1.0.0", lifespan=lifespan)
...
def _resolve_file_voice(voice: str, request: Request) -> Voice:
    if not voice.startswith(FILE_VOICE_PREFIX):
        ...
        raise HTTPException(
            status_code=422,
            detail="voice must use 'file://' prefix (OmniVoice has no built-in voices)",
        )
...
async def speech(req: SpeechRequest, request: Request) -> Response:
    ...
    voice_obj = _resolve_file_voice(req.voice, request)
...
async def clone(
    ...
    tmp_dir = Path(tempfile.gettempdir()) / "omnivoice-open-tts"
